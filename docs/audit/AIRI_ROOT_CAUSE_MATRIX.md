...
| planning panel يعرض خطوات لم تُنشأ | UI tracker كان يمكن أن يملأ placeholders بدلاً من event admitted صادر من execution | `ExecutionStatusBus` + `TaskExecutionTracker` + `ExecutionTracePresentation` | 09، 27B | executionId/actionId typed event log يخلق العنصر عند admission فقط؛ trace محدود بتسلسل مملوك وتنشره AgentPlanViewModel للواجهة | يمنع fake progress ويفصل running/fail/cancel/complete؛ CI يثبت المصدر والاختبارات، ويبقى دليل جهاز/rotation/RTL مفتوحاً |
