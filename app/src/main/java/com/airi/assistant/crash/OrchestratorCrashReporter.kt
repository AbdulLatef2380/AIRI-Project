...
class OrchestratorCrashReporter(
    ...
    fun reportAgentCrash(
        ...
        // the sanitized goal separately in the AIRI_RUNTIME line so operators can
        ...
        LoggingService.error(TAG, "AIRI_RUNTIME AGENT_CRASH agent=$agentId id=${report.id} " +
    ...
    fun reportGraphCrash(
        ...
        LoggingService.error(TAG, "AIRI_RUNTIME DAG_CRASH planId=$planId node=$nodeId id=${report.id} error=${throwable.javaClass.simpleName}")
    }
    ...
    fun reportManual(
        ...
        LoggingService.warn(TAG, "AIRI_RUNTIME MANUAL_CRASH component=$component tag=$errorTag id=${report.id}")
    }
    ...
    fun reportDurableTaskCrash(
        ...
        LoggingService.error(TAG, "AIRI_RUNTIME DURABLE_CRASH taskId=$taskId agent=$agentId id=${report.id}")
    }
}
