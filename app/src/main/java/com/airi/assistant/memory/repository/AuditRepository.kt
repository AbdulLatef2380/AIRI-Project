 ...
 * persistent AIRI audit log.
 *
 * ── , ────────────────────────────────────────────────────────
 * This class bridges the logcat-only AIRI system to Room persistence.
 * All callers that previously only wrote `Log.i(TAG, "AIRI …")` can now
...
class AuditRepository(private val db: AiriDatabase) {
     ...
     *
     * Also emits a logcat line with the canonical AIRI prefix so the
    ...
    fun log(
        ...
        val logcatMsg = "AIRI $tag $message"
        when (level) {
