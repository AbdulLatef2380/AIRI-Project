...
rag_retrieval_ranker = read('app/src/main/java/com/airi/assistant/memory/rag/RagRetrievalRanker.kt')
...
github_project_secret_test = read('app/src/test/java/com/airi/assistant/connector/app/GitHubConnectorProjectSecretTest.kt')
...
browser_takeover_coordinator = read('app/src/main/java/com/airi/assistant/agent/browser/BrowserUserTakeoverCoordinator.kt')
airi_app = read('app/src/main/java/com/airi/assistant/ui/AiriApp.kt')
project_file_edit_runtime = read('app/src/main/java/com/airi/assistant/workspace/ProjectFileEditRuntime.kt')
agent_loop = read('app/src/main/java/com/airi/assistant/agent/loop/AgentLoop.kt')
agent_loop_side_effect_policy = read('app/src/main/java/com/airi/assistant/agent/loop/tool/AgentLoopSideEffectPolicy.kt')
...
check('Long-term memory content edit UX', 'suspend fun editMemoryContent(memoryId: Long, content: String)' in memory and 'takeIf { it.isMemory } ?: return false' in memory and 'suspend fun editMemoryEntry(memoryId: Long, content: String)' in chat_vm and 'memoryManager.editMemoryContent(memoryId, content)' in chat_vm and 'editCandidate' in memory_screen and 'viewModel.editMemoryEntry(candidate.id, editDraft)' in memory_screen and 'onEdit = if (msg.isMemory)' in memory_screen and 'memory_edit_title' in memory_screen, 'Only durable-memory content is editable through the canonical manager; existing scope/provenance/project/privacy/importance/expiry are preserved and the ViewModel refreshes the visible projection.')
...
check('Scoped retrieval ranking and deduplication', 'isVisibleInScope' in rag_retriever and '.filter(::isPromptSafe)' in rag_retriever and 'RagRetrievalRanker.rank(' in rag_retriever and 'rankedPassages = RagRetrievalRanker.rank' in rag_retriever and 'filterNot(::isDurableMemory)' in rag_retrieval_ranker and 'filter(::isDurableMemory)' in rag_retrieval_ranker and 'sortedWith(' in rag_retrieval_ranker and 'contentFingerprint' in rag_retrieval_ranker and 'isLetterOrDigit' in rag_retrieval_ranker and 'it.citationId' in rag_retrieval_ranker, 'Already-authorized semantic/project passages are score-ordered and durable memory stays a governed fallback; exact normalized-content deduplication occurs only after scope/privacy/prompt-safety filters, without new admission, inferred truth, or automatic contradiction resolution.')
...
check('Scheduled task outcomes', 'lastOutcome: ScheduledJobOutcome' in scheduler and 'recordRunResult(' in worker and 'outcome = ScheduledJobOutcome.COMPLETED' in worker and 'outcome = if (canRetry) ScheduledJobOutcome.RETRYING else ScheduledJobOutcome.FAILED' in worker, 'Job results are persisted by worker.')
check('Scheduled task evidence and exact execution focus', 'jobs = jobs' in agent_tasks_screen and 'focusedExecutionId' in agent_tasks_screen and 'onOpenExecution = { taskId ->' in agent_tasks_screen and 'focusedExecutionId = taskId' in agent_tasks_screen and 'focusedTaskId = focusedExecutionId' in agent_tasks_screen and 'onClearFocus = { focusedExecutionId = null }' in agent_tasks_screen and 'tasks.firstOrNull { it.id == id }' in agent_tasks_screen and 'job.lastDurableTaskId != null' in agent_tasks_screen and 'onOpenExecution(job.lastDurableTaskId)' in agent_tasks_screen and 'execution_center_linked_run_missing' in agent_tasks_screen and 'execution_center_show_all_runs' in agent_tasks_screen and 'scheduled_task_refresh' in agent_tasks_screen and 'LaunchedEffect(selectedTab)' in agent_tasks_screen, 'Scheduled UI retains completed and failed persisted jobs, supports explicit refresh without polling, and opens the exact linked durable task with a safe return to the full execution list when that evidence still exists.')
...
check('User-confirmed scheduled run-now', 'fun runNow(jobId: String)' in scheduler and 'manualWorkName(job.id)' in scheduler and 'manualRunRequestId' in scheduler and 'job.agentId == SYSTEM_AGENT_ID' in scheduler and 'isWorkActive(existingManualId)' in scheduler and 'buildWorkerData(job, manualRun = true)' in scheduler and 'KEY_MANUAL_RUN' in worker and 'completedManualRunRequestId = manualRunRequestId' in worker and 'current[index].manualRunRequestId == completedManualRunRequestId' in scheduler and 'runNowCandidate' in agent_tasks_screen and 'orchestrator.runNow(job.id)' in agent_tasks_screen and 'scheduled_task_run_now_title' in agent_tasks_screen and 'onRunNow = if (job.agentId != "system")' in agent_tasks_screen, 'A visible user confirmation queues at most one independent immediate WorkManager execution for an agent job without replacing its schedule; maintenance jobs are refused and only the matching manual worker completion clears the persisted manual-run marker.')
...
check('AgentLoop side-effect fail-closed boundary', 'AgentLoopSideEffectPolicy.decide(' in agent_loop and 'hasDurableExecutionContext = false' in agent_loop and 'AIRI TOOL_BLOCKED_NO_DURABLE_CONTEXT' in agent_loop and 'calendar_create' in agent_loop_side_effect_policy and 'create_note' in agent_loop_side_effect_policy and 'set_alarm' in agent_loop_side_effect_policy and 'open_app' in agent_loop_side_effect_policy and 'type_text' in agent_loop_side_effect_policy and 'DURABLE_CONTEXT_REQUIRED' in agent_loop_side_effect_policy, 'Chat-owned AgentLoop calls that lack task/run/step identity fail closed before Calendar, Notes, Alarm, or accessibility side effects; read tools remain outside this write gate.')
...
check('GitHub project secret adapter', 'secretVault = secretVault' in service_locator and 'secretVault: SecretVault?' in connector_bootstrap and 'GitHubConnector(authManager, durableTaskManager, secretVault)' in connector_bootstrap and 'executeWithProjectSecret(input, execution!!)' in github_connector and 'projectExecutionOwnership(execution)' in github_connector and 'legacyCredentialProvider' in github_connector and 'tokenExecutor' in github_connector and 'vault.issueCapability(' in github_connector and 'vault.useProjectCapability(' in github_connector and 'GITHUB_PAT_SECRET_ID' in github_connector and 'project_secret_missing' in github_connector and 'missingProjectSecretFailsClosedWithoutReadingLegacyCredentialOrCallingAdapter' in github_project_secret_test and 'invalidProjectOwnershipFailsClosedWithoutLegacyFallbackOrAdapterCall' in github_project_secret_test and 'ownedProjectExecutionUsesProjectSecretInInjectedAdapterOnly' in github_project_secret_test, 'A project-owned GitHub execution validates ownership then consumes a one-use project/connector capability inside the adapter without a global-PAT fallback; a deterministic injected adapter test covers missing secret, ownership denial, and project-credential-only execution without network I/O.')
...
check('Visible browser user-takeover confirmation', 'BrowserUserTakeoverCoordinator.pending.collectAsState()' in airi_app and 'BrowserUserTakeoverCoordinator.confirm(context, request.id)' in airi_app and 'browser_takeover_title' in airi_app and 'browser_takeover_open' in airi_app and 'fun confirm(context: Context, requestId: String)' in browser_takeover_coordinator and 'BrowserNavigationPolicy.evaluate(' in browser_takeover_coordinator and 'context.startActivity(intent)' in browser_takeover_coordinator and 'ActivityNotFoundException' in browser_takeover_coordinator and 'SecurityException' in browser_takeover_coordinator, 'A visible root dialog exposes only an admitted public URL and launches it once after user confirmation; the coordinator revalidates policy and fails safely when Android has no permitted browser handler.')
check('Project file edit proposal and exact-step apply', 'data class ResumableProjectFileWrite(' in approval_continuation and 'candidateText' not in approval_continuation.split('data class ResumableProjectFileWrite(', 1)[1].split('data class ResumableConnectorInvocation', 1)[0] and 'fun applyTextRevision(' in project_file_manager and 'readTextForEdit(' in project_file_manager and 'createTextRevisionBackup(' in project_file_manager and 'projectFileWrite = ResumableProjectFileWrite(' in project_file_edit_runtime and 'pauseForApproval(' in project_file_edit_runtime and 'claimApprovedContinuation(' in project_file_edit_runtime and 'applyTextRevision(' in project_file_edit_runtime and 'restoreTextRevision(' in project_file_edit_runtime and 'linkArtifact(' in project_file_edit_runtime and 'createEvidence(' in project_file_edit_runtime and 'deleteCandidate(completed.id)' in project_file_edit_runtime and 'fun reconcileApproval(approvalId: String)' in project_file_edit_runtime and 'deleteCandidate(proposal.id)' in project_file_edit_runtime and 'approvedProjectFileWriteApprovalIds' in project_file_edit_runtime, 'Managed text edits persist private proposals, pause and claim one typed hash-only continuation, apply through project-scoped revision storage, rollback if evidence cannot link, and retain no candidate payload after completion.')
