...
| Telemetry and crash-reporting consent | `BUILD_VERIFIED` / `EXTERNAL_VERIFICATION_REQUIRED` | كان manifest يعطّل جمع Firebase افتراضياً وحارس الأحداث يمنع `logEvent`، لكن SDK لا كان يُزامن صراحة مع تبديل consent وCrashlytics كان يسمح بواجهات metadata/error حسب الاستدعاء. الآن ينعكس consent في `setAnalyticsCollectionEnabled` عند التهيئة والتغيير والسحب، ولا تسجل Analytics محلياً قيم الأحداث؛ كما يحجب `FirebaseCrashReporter` المفاتيح والسجلات والاستثناءات حتى التفعيل ويُعطّل فور السحب. حارس النواة 76/76 والتوطين و`:app:compileDebugKotlin` اجتازت. | يلزم دليل Firebase DebugView/شبكة على جهاز بتسلسل opt-in، opt-out، restart، revoke وتحقق فعلي من عدم إرسال event/crash قبل الموافقة أو بعدها؛ لا يثبت البناء قبول سياسة البيانات أو إعداد مشروع Firebase. |
