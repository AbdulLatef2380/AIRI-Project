---
name: memory_leak_auditor
description: Find retention risks and memory pressure in application code Use when the user requests memory leak auditor.
---
...
Find retention risks and memory pressure in application code
...
Use this skill when the user's request matches `memory_leak_auditor` or its documented capability. Confirm the input scope, expected output, privacy level, and required connector or permission before execution.
...
- Ask AIRI to memory leak auditor using supplied evidence.
