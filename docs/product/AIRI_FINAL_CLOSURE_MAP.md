...
| P0-05 | Project secrets / `SecretVault` | `PARTIAL` | Keystore-backed capability broker is live. | Agent/operation/TTL/use binding exists; `projectId` ownership and connector/task scope closure are open. | No project-secret management UI. | Add ProjectSecret isolation, revoke, expiry and connector-scope tests. | Keystore/device coverage. |
