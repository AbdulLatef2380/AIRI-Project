...
| RC-B03 | Approval and exact-step continuation | `TrustCenter`, typed continuation runtime, durable task product kernel | CI instrumentation confirms one approved local continuation resumes once after runtime reconstruction; denial and UI-driven replay proof remain. | `PARTIAL` / `CI_VERIFIED` |
...
| 2026-08-24 — Local approval/recovery journey | The reference local file path now has Android integration coverage across task ownership, approval, runtime recreation, one apply, provenance, and duplicate-resume refusal. | `ProjectFileApprovalRecoveryTest` creates a managed project file and running owned task, persists a private proposal and approval, recreates the durable/workspace/file/artifact/proposal runtimes, resumes exactly once, verifies project/task/run/step artifact provenance, and confirms no second recovery resume. | Physical-device UI approval/denial, deliberate process kill, TalkBack, and full Chat→Tasks→Trust→Library traversal remain. | Extend only the defined journey; do not introduce provider or browser scope. | `ProjectFileApprovalRecoveryTest.kt`. | Core guard 76/76, strict localization, Android CI `32695492715`: debug/unit/lint/release-source, API 29 instrumentation, and native verification passed. | `73ac96ad` | This is CI emulator evidence, not signed artifact, physical-device, provider, or store proof. |
