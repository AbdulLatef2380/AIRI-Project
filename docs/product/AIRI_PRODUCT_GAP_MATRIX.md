...
| T0-03 | File Intelligence | تحويل المرفق إلى مورد قابل للبحث والمعرفة | `PARTIAL` | Project File حقيقي: URI→validate→SHA-256→managed storage→preview، وفهرسة نصية محلية صريحة؛ fixture A/B المجمّعة تمرر import→index→search وتثبت أن معرفة A لا تظهر في B، وmetadata المملوك فقط يدخل Project Context بلا path/URI/hash/raw content؛ المتبقي تشغيل Android fixture، semantic index وversion/restore وdocument parsers وpicker | Android/Desktop | استيراد، بحث exact/semantic، حذف واستعادة |
