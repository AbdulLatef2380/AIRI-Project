...
|---|---|---|---|---|---|---|
| T0-01 | Workspace 2.0 | مشروع واحد يجمع الوكيل والملفات والمهام والمعرفة والأدوات | `PARTIAL` | Product Kernel يطبع Mission/Task/Run/Step/Approval ownership في التخزين الدائم، و`ProjectContextResolver` يضيف metadata/files/artifacts المملوكة فقط لمسار RAG؛ artifacts تحفظ provenance، وSecret Manager يهيئ/يبطل `GITHUB_PAT` للمشروع النشط بحالة presence فقط، ثم يستهلكه GitHub مقيّداً بالمشروع/connector بعد تحقق task/run/step بلا fallback عالمي؛ المتبقي ترحيل بقية producers/providers واختبار الاستعادة الميداني | Android/Desktop | إنشاء مشروع، فتحه، واستعادة سياقه بعد إعادة التشغيل |
| T0-02 | Project Context | عدم إعادة شرح المشروع في كل محادثة | `PARTIAL` | `ChatViewModel` يمرر projectId النشط إلى RAG؛ `ProjectContextResolver` يقبل metadata/files/artifacts المطابقة فقط ضمن budget صريح، والمعرفة/الذاكرة تبقيان في RAG المقيد بالـscope/privacy؛ fixture A/B المجمّعة يغطي file/knowledge/memory/secret/artifact boundaries؛ المتبقي Android visual/runtime وprovider/artifact-task isolation الميداني | Android/Desktop | الرسالة الجديدة ترى موارد المشروع المصرح بها فقط |
| T0-03 | File Intelligence | تحويل المرفق إلى مورد قابل للبحث والمعرفة | `PARTIAL` | Project File حقيقي: URI→validate→SHA-256→managed storage→preview، وفهرسة نصية محلية صريحة، وسلة خاصة بالمشروع delete→restore→purge تنظف knowledge ولا تعيد فهرسته تلقائياً؛ محرر نص محلي يكتب proposal خاصاً وdiff محدوداً ثم يطلب approval durable قبل backup/temp-replace/recovery المقيد بالمشروع، ويعيد index إلى `NOT_REQUESTED`؛ fixture A/B المجمّعة تغطي import→index→delete→restore→reindex وتثبت أن معرفة A لا تظهر في B، وfixture revision جديدة تُجمع فقط؛ المتبقي تشغيل Android fixture، semantic index وcontent versioning متعدد المستخدمين وdocument parsers وpicker/device usability | Android/Desktop | استيراد، بحث exact/semantic، حذف واستعادة، وتعديل نصي بموافقة صريحة |
| T0-04 | Memory Fabric | ذاكرة قابلة للتفسير والحذف وليست مخزناً عشوائياً | `PARTIAL` | Room v8 يطبق provenance/scope/privacy/expiry وMemoryAgent لا يعلن تخزيناً كاذباً؛ RAG يمنع `PROJECT` memory غير المطابقة للمشروع قبل بناء prompt، وfixture A/B المجمعة تكتب MemoryManager في A وتثبت أن B لا يستدعيها؛ MemoryScreen تعرض سبب الحفظ والنطاق/الثقة وتؤكد حذف ذاكرة طويلة فردية عبر ViewModel مع تحديث القائمة والعداد؛ المتبقي تشغيل fixture، طبقات Working/Episodic/Semantic الكاملة والتصدير وmigration/accessibility على جهاز | Core/Android/Desktop | عرض المصدر، التصحيح، الحذف، والتصدير |
...
| T0-06 | Execution Center | فهم ما فعله الوكيل ولماذا | `PARTIAL` | Durable Task timeline يسجل Mission/Task/Run/Step/tool/recovery/error مع تطبيع JSON للملكية؛ نجاح خطوة `ProductionAgentOrchestrator` ينشئ artifact خاصاً مملوكاً لـproject/task/run/step ويربطه بحدث timeline، وLibrary تعرض task/run/step/tool/model/summary وبادئة hash محدودة؛ `cancelAll()` يوقف scope الخطة الحالية ثم يهيئ scope جديداً، فلا يعطل الخطط التالية؛ المتبقي event detail/retry وبقية artifact producers وprocess death/accessibility على جهاز | Android/Desktop | replay قابل للقراءة وإعادة الفتح |
| T0-07 | Approval Center | موافقة واضحة قبل الأثر الجانبي | `PARTIAL` | موافقة Mission/Task/Run/Step دائمة مع Allow once/task ورفض/انتهاء واستعادة القرار؛ GitHub mutation المقيد بالمهمة وكتابة Project File المحلية يحفظان continuations typed آمنة ثم يوقفان نفس run/step ويُclaimان مرة واحدة قبل API/file I/O، وTrust Center يوجه الاستئناف للـruntime المطابق؛ AgentLoop chat بلا task/run/step صار يحجب Calendar/Notes/Alarm/Accessibility/App-intent side effects قبل adapter بدل قبول sentinel حواري كإذن؛ المتبقي task-owned AgentLoop context وcalendar continuation typed، والمهارات والطرفية وبقية الموصلات وAllow project enforcement واختبار جهاز/provider | Android/Desktop | لا ينفذ command أو secret أو connector أو كتابة ملف قبل القرار |
