...
description: Model threats and mitigations for a feature or system Use when the user requests security threat model.
---
...
Model threats and mitigations for a feature or system
...
Use this skill when the user's request matches `security_threat_model` or its documented capability. Confirm the input scope, expected output, privacy level, and required connector or permission before execution.
...
- Ask AIRI to security threat model using supplied evidence.
...
Apply the defensive-only boundary: work only on supplied or explicitly authorized lab evidence; do not generate exploit payloads or operational attack instructions.
