...
|---|---|---|---|
| توقيع الإصدار | `build.gradle.kts` يقرأ `KEYSTORE_BASE64` وبياناته من البيئة فقط، وينشئ `release.keystore` محلياً عند توافرها؛ `.gitignore` يستثني `*.jks` و`*.keystore`. بعد ترقية المرشح `fe3fb68b` إلى `main`، GitHub Actions run `32742046966` اجتازت الحارس والبناء والاختبارات وinstrumentation/native، لكن بوابة signing أعلنت `RELEASE_SIGNING_READY=false`: لم تُنشأ APK/AAB موقعة لأن واحداً أو أكثر من `KEYSTORE_BASE64` و`STORE_PASSWORD` و`KEY_ALIAS` و`KEY_PASSWORD` غير مهيأ. | `SIGNING_SECRETS_BLOCKED`؛ لا يوجد توقيع main حقيقي أو artifact نشر. | يهيئ release owner الأسرار الأربعة عبر قناة GitHub آمنة، ثم يعيد main CI ويحفظ APK/AAB الموقعين و`mapping.txt` و`SHA256SUMS` ومخرجات apksigner. لا توضع مفاتيح أو قيم أسرار في المستودع أو السجلات. |
| تقليص وحماية APK | `release` يفعّل R8 وresource shrinking، ويحتوي على فحص JNI مخصص لكل APK. على غير `main` تبني CI `assembleRelease`/`bundleRelease` وتتحقق من `mapping.txt` وAPK badging وAAB ZIP و`UNSIGNED_SHA256SUMS`. run `32720458806` نجحت؛ evidence المفحوص يحمل APK غير موقّع SHA-256 `20c6…a236` وAAB `6471…9dcf` وmapping `d82a…d5a7`، مع native `arm64-v8a` وعدم وجود Billing في badging. بوابة main الموقعة تبقى مسؤولة عن `apksigner` وSHA-256 النهائي. | `CI_UNSIGNED_PACKAGE_VERIFIED` / `EXTERNAL_SIGNING_REQUIRED`. | شغّل release assembly في CI المحمي؛ اجمع artifact الموقع وmapping وSHA-256 ونتيجة apksigner. لا تستخدم hashes غير الموقعة كهوية ناشر أو دليل نشر. |
...
| R8 entry point | قاعدة ProGuard تحتفظ الآن بـ`com.airi.assistant.app.AIRIApplication` المطابق للـmanifest. تعبئة R8 غير الموقعة في run `32720458806` أنتجت `mapping.txt` من 815,811 سطراً ومرّت APK/AAB verification. | `IMPLEMENTED` / `CI_UNSIGNED_PACKAGE_VERIFIED`. | إثبات artifact الموقع وinstall/runtime على جهاز ما زال مطلوباً. |
