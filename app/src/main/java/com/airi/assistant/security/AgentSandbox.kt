 ...
 *   5. Logs every entry/exit with AIRI_RUNTIME tag for audit.
 *
 * ── WORKSPACE ─────────────────────────────────────────────────────────────
 *
...
class AgentSandbox(
    ...
    suspend fun <T> execute(
        ...
        LoggingService.info(TAG, "AIRI_RUNTIME SANDBOX_ENTER agent=$agentId goal=$goalId required=$requiredPermissions")

        requiredPermissions.forEach { perm ->
            if (!registry.check(agentId, perm)) {
                val msg = "Sandbox denied: agent=$agentId missing permission=$perm"
                LoggingService.warn(TAG, "AIRI_RUNTIME SANDBOX_DENIED agent=$agentId perm=$perm")
        ...
        return try {
            ...
            LoggingService.info(TAG, "AIRI_RUNTIME SANDBOX_EXIT_OK agent=$agentId goal=$goalId")
            result
        } catch (e: ScopedPermissionRegistry.PermissionDeniedException) {
            LoggingService.warn(TAG, "AIRI_RUNTIME SANDBOX_RUNTIME_VIOLATION agent=$agentId msg=${e.message}")
            ...
            LoggingService.warn(TAG, "AIRI_RUNTIME SANDBOX_UNKNOWN_TOOL agent=$agentId msg=${e.message}")
