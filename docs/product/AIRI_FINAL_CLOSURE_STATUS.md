...
| Cross-project file/knowledge/memory/artifact fixture | `BUILD_VERIFIED` | `ProjectResourceIsolationTest` compiles against real ProjectFileManager, ProjectKnowledgeManager, MemoryManager, and ArtifactManager paths; it asserts A/B file visibility, knowledge search, project-scoped memory recall, soft-delete/restore/reindex, and artifact lookup/read denial. | Execute the instrumentation fixture on a physical device/emulator and extend it with project-secret capability. |
