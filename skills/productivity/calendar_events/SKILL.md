...
description: Official AIRI skill for calendar events. Use when the user requests calendar events.
---
...
Official AIRI skill for calendar events.
...
Use this skill when the user's request matches `calendar_events` or its documented capability. Confirm the input scope, expected output, privacy level, and required connector or permission before execution.
...
- Ask AIRI to calendar events using supplied evidence.
