...
|---|---|---|---|---|---|---|
| T0-01 | Workspace 2.0 | مشروع واحد يجمع الوكيل والملفات والمهام والمعرفة والأدوات | `PARTIAL` | Product Kernel يربط Workspace/Task/Project Files/Artifacts؛ `ProjectContextResolver` يضيف metadata/files/artifacts المملوكة فقط لمسار RAG، وتبقى الذاكرة/المعرفة محكومة بـscope/privacy؛ المتبقي Mission aggregate، secret project ownership، واختبار الاستعادة على جهاز | Android/Desktop | إنشاء مشروع، فتحه، واستعادة سياقه بعد إعادة التشغيل |
...
| T0-03 | File Intelligence | تحويل المرفق إلى مورد قابل للبحث والمعرفة | `PARTIAL` | Project File حقيقي: URI→validate→SHA-256→managed storage→preview، وفهرسة نصية محلية صريحة؛ metadata المملوك فقط يدخل Project Context بلا path/URI/hash/raw content؛ المتبقي semantic index وversion/restore وdocument parsers واختبار Android picker | Android/Desktop | استيراد، بحث exact/semantic، حذف واستعادة |
...
| T0-06 | Execution Center | فهم ما فعله الوكيل ولماذا | `PARTIAL` | Durable Task timeline يسجل run/step/tool/recovery/error مع واجهة replay مختصرة؛ `cancelAll()` يوقف scope الخطة الحالية ثم يهيئ scope جديداً، فلا يعطل الخطط التالية؛ المتبقي event detail/retry وartifact links واختبار process death على جهاز | Android/Desktop | replay قابل للقراءة وإعادة الفتح |
| T0-07 | Approval Center | موافقة واضحة قبل الأثر الجانبي | `PARTIAL` | موافقة Task/Run/Step دائمة مع Allow once/task ورفض/انتهاء واستعادة القرار؛ يعرض Trust Center الحي طلبات المهمة وطلبات الحوكمة غير المرتبطة بمهمة مع action/risk/run/step ويعيد القرار عبر `PermissionGovernanceLayer`؛ المتبقي Allow project enforcement وتعديل الأمر وcontinuation لاستئناف الأداة بعد القرار واختبار جهاز | Android/Desktop | لا ينفذ command أو secret أو connector قبل القرار |
...
| T0-10 | Artifact System | نتيجة قابلة للمعاينة والتنزيل وإعادة التوليد | `PARTIAL` | ملفات معرّفة بوضوح وpreview معزول وsnapshot versions/restore فعلي؛ resolver يعرض metadata artifact للـsession المطابق فقط؛ المتبقي compare/hash/export/download وربط artifact بسجل التنفيذ task/run/step | Core/Android/Desktop | إنتاج ملف، معاينته، حفظه، وتنزيله |
