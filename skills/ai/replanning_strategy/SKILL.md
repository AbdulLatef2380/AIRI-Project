---
name: replanning_strategy
description: Generate a bounded recovery plan after a failed or low-confidence step Use when the user requests replanning strategy.
---
...
Generate a bounded recovery plan after a failed or low-confidence step
...
Use this skill when the user's request matches `replanning_strategy` or its documented capability. Confirm the input scope, expected output, privacy level, and required connector or permission before execution.
...
- Ask AIRI to replanning strategy using supplied evidence.
