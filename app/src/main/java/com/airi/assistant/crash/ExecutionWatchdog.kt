...
class ExecutionWatchdog(
    ...
    fun start() {
        ...
        isRunning = true
        scope.launch {
            LoggingService.info(TAG, "AIRI_RUNTIME WATCHDOG_STARTED interval=${SCAN_INTERVAL_MS}ms threshold=${STUCK_THRESHOLD_MS}ms")
            while (isActive) {
    ...
    private fun scan() {
        ...
        for (snap in snapshots) {
            ...
            LoggingService.warn(TAG, "AIRI_RUNTIME WATCHDOG_STUCK_DETECTED planId=${snap.planId} ageMs=$age")
            ...
            if (autoCancelStuck) {
                LoggingService.warn(TAG, "AIRI_RUNTIME WATCHDOG_AUTO_CANCEL planId=${snap.planId}")
