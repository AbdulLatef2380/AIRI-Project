...
| Android CI: instrumentation runtime | `TRIAGE_ROOT_CAUSE_IDENTIFIED` / `RERUN_REQUIRED` | artifact `AIRI-Android-Test-Reports` من run `32675745165` حدد الاختبارين: `ProjectResourceIsolationTest.projectResourcesNeverCrossProjectBoundary` و`textRevisionRemainsProjectOwnedAndClearsStaleKnowledge`. كلاهما استورد نصاً صريحاً أقصر من `MIN_CHUNK_CHARS=80`؛ `ProjectKnowledgeTextPolicy` أسقطه بلا مقطع، فعاد `IndexStatus.FAILED`. أصلح المصدر الآن للاحتفاظ بمقطع lexical واحد للنص القصير غير الفارغ المختار صراحةً، وأضاف اختبار JVM انحدارياً وحارس مصدر. | لا يجوز الادعاء بنجاح instrumentation قبل أن يمر تشغيل CI على هذا الإصلاح. لا يعالج التغيير فهرسة فارغة أو إدخالاً تلقائياً؛ النص الفارغ يبقى بلا مقطع. |
...
3. إعادة تشغيل `connectedDebugAndroidTest` في CI على إصلاح النص القصير؛ لا تعاد تسمية النتيجة نجاحاً قبل مرور الاختبارين وفحص native artifact فعلياً.
