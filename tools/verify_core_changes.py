...
check('Artifact execution provenance', 'version = 9' in database and 'MIGRATION_8_9' in database and 'val projectId:' in artifact_entity and 'Index(value = ["projectId", "taskId", "runId", "stepId"])' in artifact_entity and 'getByIdForProject' in artifact_dao and 'validateProvenance(' in artifact_manager and 'getArtifactForProject' in artifact_manager and 'contentHash' in artifact_manager and 'persistStepArtifact(' in production_orchestrator and 'linkArtifact(plan.id, artifact.id, plan.id, task.id)' in production_orchestrator and 'fun linkArtifact(taskId: String, artifactId: String, runId: String, stepId: String)' in durable_task_manager, 'Artifacts persist project/task/run/step/tool/model metadata with integrity hash, enforce project ownership, and are created/linked from successful orchestrator steps.')
check('Cross-project resource isolation fixture', 'files.importFromBytes(' in project_resource_isolation_test and 'knowledge.indexProjectFile(' in project_resource_isolation_test and 'knowledge.search(projectB' in project_resource_isolation_test and 'getArtifactForProject(artifactA.id, projectB)' in project_resource_isolation_test and 'readContentForProject(artifactA.id, projectB)' in project_resource_isolation_test, 'Instrumentation fixture composes real project file import, local knowledge retrieval, and artifact project reads to reject cross-project access.')
