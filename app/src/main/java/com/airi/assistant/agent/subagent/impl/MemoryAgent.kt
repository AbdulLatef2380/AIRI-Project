...
class MemoryAgent(
    ...
    private fun executeStore(input: String, start: Long) = flow<AgentEvent> {
        ...
        Log.i(TAG, "AIRI_PROOF MEMORY_STORED content='${content.take(80)}'")
    ...
    private fun executeRecall(input: String, context: SubAgentContext, start: Long) = flow<AgentEvent> {
        ...
        val memories = if (memoryManager.isSemanticMemoryReady()) {
            ...
            Log.d(TAG, "AIRI_PROOF MEMORY_RECALL semantic hits=${hits.size}")
            if (hits.isNotEmpty()) {
                hits.map { it.message }
            } else {
    ...
    private fun executeDelete(start: Long) = flow<AgentEvent> {
        ...
        Log.i(TAG, "AIRI_PROOF MEMORY_CLEARED all episodic memories deleted")
        emit(AgentEvent.PartialResult(
