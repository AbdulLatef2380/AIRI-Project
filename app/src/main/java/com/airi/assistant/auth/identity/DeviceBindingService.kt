...
class DeviceBindingService(private val context: Context) {
    ...
    fun bindOrThrow(): String {
        ...
        return when {
            stored == null -> {
                ...
                LoggingService.info(TAG, "AIRI_RUNTIME DEVICE_BIND_INITIAL fingerprint=${current.take(12)}…")
                current
            }
            stored == current -> {
                LoggingService.info(TAG, "AIRI_RUNTIME DEVICE_BIND_OK fingerprint=${current.take(12)}…")
                current
            }
            else -> {
                LoggingService.warn(TAG, "AIRI_RUNTIME DEVICE_BIND_MISMATCH stored=${stored.take(12)}… current=${current.take(12)}…")
    ...
    fun clearBinding() {
        ...
        LoggingService.info(TAG, "AIRI_RUNTIME DEVICE_BIND_CLEARED")
    }
