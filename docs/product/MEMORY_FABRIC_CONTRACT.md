...
The next knowledge phase must connect an explicit Project File index request to a real extractor/indexer, preserve source chunks and confidence, and expose those citations in the same evidence model. Importing a file must not become implicit long-term memory or knowledge. Physical-device verification remains required for the delete/edit dialogs, TalkBack labels, font-scale rendering, validation failures, and persistence refresh after process recreation. Memory taxonomy expansion and user export remain separate gaps; no export surface is claimed by this contract.
