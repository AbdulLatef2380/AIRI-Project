 ...
 * logs an explicit AIRI_RUNTIME EMBEDDING_NOT_LOADED — the chat path then
...
class EmbeddingService(context: Context) {
     ...
     * success, or null on failure (with an AIRI_RUNTIME entry explaining
    ...
    suspend fun loadEmbeddingModel(modelPath: String): Int? = withContext(nativeDispatcher) {
        nativeLock.withLock {
            ...
            if (!f.exists()) {
                Log.i("AIRI_RUNTIME", "EMBEDDING_MODEL_LOAD_FAILED reason=file_not_found path=$modelPath")
                return@withLock null
            }
            if (!LlamaNative.isAvailable()) {
                Log.i("AIRI_RUNTIME", "EMBEDDING_MODEL_LOAD_FAILED reason=native_lib_missing")
                return@withLock null
            }
            val rc = runCatching { LlamaNative.loadEmbeddingModel(modelPath) }
                .getOrElse {
                    Log.i("AIRI_RUNTIME", "EMBEDDING_MODEL_LOAD_FAILED reason=jni_throw msg=${it.message}")
                    return@withLock null
                }
            // Native returns "OK dim=NNN" on success, "ERR_*" on failure.
            if (!rc.startsWith("OK")) {
                Log.i("AIRI_RUNTIME", "EMBEDDING_MODEL_LOAD_FAILED reason=$rc")
                return@withLock null
            }
            val parsedDim = rc.substringAfter("dim=", "0").toIntOrNull() ?: 0
            if (parsedDim <= 0) {
                Log.i("AIRI_RUNTIME", "EMBEDDING_MODEL_LOAD_FAILED reason=bad_dim raw=$rc")
                return@withLock null
            }
            loadedPath = modelPath
            dim = parsedDim
            Log.i("AIRI_RUNTIME", "EMBEDDING_MODEL_LOADED dim=$parsedDim path=${f.name}")
            parsedDim
        }
    }
    ...
    suspend fun unload() = withContext(nativeDispatcher) {
        nativeLock.withLock {
            if (loadedPath != null) {
                ...
                Log.i("AIRI_RUNTIME", "EMBEDDING_MODEL_UNLOADED path=${loadedPath?.substringAfterLast('/')}")
            }
            loadedPath = null
            dim = 0
        }
    ...
    suspend fun embedAndStore(message: ChatMessage): Boolean {
        if (!isReady()) {
            Log.i("AIRI_RUNTIME", "EMBEDDING_SKIPPED reason=not_loaded msg_id=${message.id}")
            return false
        }
    ...
    suspend fun topKSimilar(
        ...
        if (!isReady()) {
            Log.i("AIRI_RUNTIME", "VECTOR_SEARCH_SKIPPED reason=not_loaded session=$sessionId")
            return emptyList()
        }
    ...
    fun formatContext(hits: List<RankedMessage>): String {
        ...
        return buildString {
            ...
            Log.i("AIRI_RUNTIME", "CONTEXT_AUGMENTED hits=${hits.size} bytes=${it.length}")
        }
    }
     ...
     *
     * Emits AIRI_RUNTIME CONTEXT_INJECTED hits=X tokens=Y on success,
    ...
    private suspend fun computeRaw(text: String): FloatArray? = withContext(nativeDispatcher) {
        nativeLock.withLock {
            ...
            val raw = runCatching { LlamaNative.computeEmbedding(text) }
                .getOrElse {
                    Log.i("AIRI_RUNTIME", "EMBEDDING_FAILED reason=jni_throw msg=${it.message}")
                    return@withLock null
                }
            if (raw == null || raw.isEmpty()) {
                Log.i("AIRI_RUNTIME", "EMBEDDING_FAILED reason=null_or_empty")
                return@withLock null
            }
            raw
        }
