...
This document describes the AIRI connector catalog entry for **Monday.com**. The entry is an AIRI adapter definition, not a claim that AIRI is the provider or owner of the external service.
...
The declared authentication family is `OAUTH2_AND_API`. AIRI must use the provider's official authorization or credential mechanism; it must never request the provider password. Credentials must remain in encrypted platform storage and must not appear in logs, UI state, Git, or build artifacts.
