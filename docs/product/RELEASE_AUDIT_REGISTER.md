...
| Initial permission and deep-link entry | `BUILD_VERIFIED` / `RUNTIME_VERIFICATION_PENDING` | أزيل طلب `POST_NOTIFICATIONS` التلقائي من `MainActivity`؛ بقي الطلب داخل إجراء Onboarding الصريح. كما بات `ReferralManager` يقبل فقط `airi://referral?code=` الصحيح ولا يلتقط `code` من URI عام أو callback OAuth، مع عدم تسجيل قيمة الإحالة الخام. | يتطلب التحقق منح/رفض الإشعارات من Onboarding على جهاز، وتجربة cold/warm deep link ورابط OAuth حقيقي؛ لا يثبت ذلك عقد مزود OAuth أو برنامج إحالة/مدفوعات خارجي. |
