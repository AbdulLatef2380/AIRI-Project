...
class OverlayService : Service() {
    ...
    override fun onCreate() {  
        super.onCreate()  
        val llamaManager = LlamaManager(this)
        val intentRouter = IntentRouter()
        cognitiveLoop = UnifiedCognitiveLoop(this, intentRouter, llamaManager)
    ...
    private fun sendToAIRI(userText: String) {  
        ...
        // إضافة فقاعة رد فارغة للبدء في التحديث المتدفق
        val assistantMsg = ChatModel("", false)
        adapter.addMessage(assistantMsg)
        val msgIndex = adapter.itemCount - 1

        cognitiveLoop.processStream(
            input = userText,
            onToken = { token ->
                mainHandler.post {
                    adapter.updateMessage(msgIndex, adapter.getMessage(msgIndex).content + token)
                }
            },
            onComplete = { fullResponse ->
                mainHandler.post {
                    ttsManager.speak(fullResponse, TextToSpeech.QUEUE_FLUSH, null, "AIRI")
                }
            }
        )
    }  
