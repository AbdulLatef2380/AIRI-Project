 ...
 *
 * All decisions emit AIRI_RUNTIME log tags so they are visible in the standard
 * `adb logcat | grep AIRI_RUNTIME` audit stream used throughout the project.
 *
...
class RuntimeSupervisor(
    ...
    fun stop() {
        ...
        Log.i("AIRI_RUNTIME", "SUPERVISOR_STOP was_active=$wasActive")
    }
    ...
    private fun readThermalMode(): PerformanceMode {
        ...
        return try {
            ...
            Log.i("AIRI_RUNTIME", "SUPERVISOR_THERMAL status=$status")
            when {
     ...
     * in AIRI_RUNTIME logs and in the consumer callback. Re-samples the
