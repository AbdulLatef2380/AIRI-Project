...
class OverlayService : Service() {
    ...
    // نستخدم LlamaManager الذي أنشأناه لربط الـ JNI
    ...
    private fun loadBrain() {
        ...
        btnLoadBrain.text = "جاري تحضير المحرك..."
        ...
        llamaManager.initializeModel { success ->
            ...
            if (success) {
                ...
                btnLoadBrain.text = "فشل التحميل (تأكد من وجود الملف)"
    ...
    private fun sendToAIRI(text: String) {
        ...
        // استخدام LlamaManager بدلاً من ModelManager
        llamaManager.generate(text) { response ->
            ...
            val recycler = chatView.findViewById<RecyclerView>(R.id.chat_recycler)
            recycler.smoothScrollToPosition(adapter.itemCount - 1)
        }
    }

    // ... (بقية دوال SpeechToText و TouchListener تبقى كما هي)
