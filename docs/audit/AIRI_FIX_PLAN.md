...
**حالة milestone الحالية: PARTIALLY_DONE / MITIGATED.** أضاف `e4559f2f` lifecycle مملوكاً للأداة (`executionId` و`actionId` وsequence وduration وterminal واحد) إلى AgentLoop، وربط ToolDispatcher بالهوية الصريحة، ووفّر overload متوافقاً خلفياً لـAdaptiveGraphEngine. ثم أضاف `380c0a54` واجهة trace منظمة في لوحة الخطة، بتصفية All/Planning/Steps/Tools/Issues وتفاصيل آمنة وتحكم متابعة/قفل وقَفز إلى الأحدث وموارد ar/en/es/zh. كشف Deep Audit `33120815009` خطأ Compose فعلياً ولم يُخفَ؛ أصلحه `064f9034`، ونجحت بوابات Android `33121217462` وDeep `33121217471` وArchitecture `33121217381` وOracle `33121217398`. وأغلقت الدفعة اللاحقة `d84f4390`/`215c4a4c`/`d0173d90` admission validator للخطط غير القابلة للتنفيذ مع CI كامل أخضر. ما زالت مسارات SkillService القديمة، واسترداد process، وإثبات RTL/font-scale والجهاز/الموفر الحي مفتوحة؛ لذلك لا تُعلن متطلبات trace كاملةً **DONE**.

## سجل تنفيذ M6 — Google connector lifecycle

أصلحت هذه الدفعة سبباً داخلياً مثبتاً في مسار Google: كان `IntegrationsViewModel` ينشئ `GoogleAuthService` خاصاً بينما يستعمل `GoogleConnector` المسجل مثيل `ServiceLocator.googleAuthService`؛ ولأن access token لبيانات Google ذاكرة فقط، لم يكن التفويض يصل إلى الموصل. في commit `c61d71316610944ab4bcbb226e0b6cb40b1d7499` أصبح المالك واحداً، ويعيد ViewModel تقييم `ConnectorState` بعد sign-in أو consent أو cancel أو disconnect، ويختار زر Google طلب التفويض بدلاً من بدء sign-in مرة ثانية عندما تكون الهوية موجودة. كما صُفّر الرمز الذاكري عند تبديل الهوية، وقُصرت الصلاحيات على حزمة قراءة موحّدة، وحُجبت `gmail_send` و`calendar_create` قبل الشبكة حتى يتوفر مسار موافقة durable قابل للمراجعة. أضيفت اختبارات انحدار لقرار الزر ولحجب أفعال الكتابة.

فشل Android CI الأول للـcommit `c61d7131` بسبب ترقية `play-services-auth` إلى `22.0.0`، التي لم تعد توفر `GoogleSignIn` الذي تستخدمه شاشات الدخول القائمة؛ مرّ تحقق hashes قبل ظهور هذا التشخيص. أثبت فحص AAR الرسمي أن `20.7.0` يوفر كلاً من `GoogleSignInClient` و`AuthorizationClient`، لذلك أعاد commit `7a24d7234e92b731ba86a7a139e2e4d4e5b64be3` التبعية إلى `20.7.0` وأزال hashes غير اللازمة وصحح كشف نوع readiness داخلي في عقد UI عام. اجتازت البوابة النهائية Android CI `33133058954` وDeep Audit `33133058955` وArchitecture Audit `33133058940` وOracle `33133058947`؛ كما اجتاز Desktop Windows `33133058958`. نجحت إعادة instrumentation في Android CI بتسعة اختبارات بعد أن أعاد workflow محاولة محاكي بدأ بحالة API غير صالحة؛ لا يمثل ذلك دليلاً على جهاز ARM64 حقيقي.

**الحالة:** Google integration `PARTIALLY_DONE` / `CI_VERIFIED`. ما زال تحقق Google Sign-In وconsent/cancel/revoke وتبديل الحساب وFirebase SHA والـprovider runtime يتطلب حساب اختبار مفوضاً وجهاز ARM64 فعلياً، ولذلك يبقى `EXTERNAL_PENDING`. لا يُعلن M6 أو الإطلاق كاملاً بسبب نجاح CI وحده.

## سجل تنفيذ M8 — إعدادات وتخصيص الصوت

تعالج هذه الدفعة شريحة محددة من M8 وM5، ولا تدّعي إغلاقهما. نقل commit `2863890533764cfc198d6260759574b216b18609` جميع النصوص المرئية المستهدفة من `SettingsScreen` و`VoicePersonalizationScreen` إلى الموارد المتكافئة `ar/en/es/zh`، واستبدل الألوان الصريحة في الإعدادات برموز `AiriTheme` الدلالية، وأزال وصف appearance الثابت غير المستمد من الحالة. كما جعل قائمة TTS تختار الأصوات المحلية المطابقة للغة التطبيق فقط وتستبعد الأصوات المعتمدة على الشبكة، وفصل حالات التهيئة وعدم التوفر وغياب صوت offline. لا تحفظ `VoicePreferencesStore` بعد الآن عبارات أو رموزاً مرئية إنجليزية؛ فهي تحتفظ فقط بمعرف preset وقيم الصوت. أضيف `VoiceLocalePolicyTest` لحالات اللغة الإقليمية والصوت الشبكي واللغة المختلفة أو الفارغة.
