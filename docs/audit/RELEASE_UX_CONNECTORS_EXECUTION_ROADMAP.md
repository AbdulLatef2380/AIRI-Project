...
| الصوت والنماذج والمهام | تخصيص TTS كان يقصر الأصوات على الإنجليزية ويعرض حالة فارغة قبل اكتمال تهيئة المحرك | قائمة الصوت تقبل أصوات لغة التطبيق المحلية فقط، تستبعد network-only، وتفصل initializing/unavailable/no-offline-voice؛ بقية عقود النماذج والمهام ما زالت مفتوحة | `PARTIALLY_DONE` / `CI_VERIFIED` |
...
| إعدادات عامة | نصوص صريحة إنجليزية لـAI execution/secure storage/about/internal surfaces، ووصف appearance ثابت غير مستمد من الحالة | `28638905`: موارد `ar/en/es/zh` متكافئة، تحذير تخزين دلالي، وإزالة وصف theme غير المثبت؛ الأسهم الاتجاهية تستعمل `AutoMirrored` المدعوم | فحص parity محلي وAndroid CI `33138024292`؛ Force RTL/light/dark/TalkBack على جهاز حقيقي |
