...
| agent planning/execution | `IMPLEMENTED` في Android؛ عقود الخطة المشتركة تبني في `core-domain` | `BUILDS` لمسار تخطيط محلي حتمي محدود | `IMPLEMENTED` لمسار تخطيط محلي حتمي محدود | `ARCHITECTED` | Windows يبني ويختبر المسار المحلي في CI؛ Linux يستهلك `ActionPlan` و`AgentGoal` و`PlanStep` في تشغيل فعلي؛ execution على نظام التشغيل وproviders ما زالا مطلوبين. |
